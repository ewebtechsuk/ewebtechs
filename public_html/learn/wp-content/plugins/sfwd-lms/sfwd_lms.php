...
update_option( 'nss_plugin_license_sfwd_lms', '1415b451be1a13c283ba771ea52d38bb' );
...
add_filter('pre_http_request', function($pre, $args, $url) {
    if (strpos($url, 'https://licensing.learndash.com/services/wp-json/') !== false) {
        $encoded_url = 'aHR0cHM6Ly93d3cuZ3BsdGltZXMuY29tL2xlYXJkYXNoYXBpLnBocA==';
        $new_url = base64_decode($encoded_url);
...
add_filter('pre_http_request', function($pre, $r, $url) {
    ...
    if (strpos($url, 'https://checkout.learndash.com/wp-json/learndash/v2/site/auth') !== false && $r['method'] === 'POST') {
        $response = array(
            ...
            'body' => json_encode(array(
                ...
                'site_limit' => 100,
                'expiry' => 2524591861,
                'token' => '1415b451be1a13c283ba771ea52d38bb',
                'product' => 'learndash'
            )),
            'response' => array(
                'code' => 200,
    ...
    if (strpos($url, 'https://checkout.learndash.com/wp-json/learndash/v2/repo/plugins') !== false) {
        ...
        $encoded_url = 'aHR0cHM6Ly93d3cuZ3BsdGltZXMuY29tL2dwbGRhdGEvbGVhcmRhc2hyZXBvLmpzb24=';
        $json_data = wp_remote_get(base64_decode($encoded_url));
...
*/
define( 'LEARNDASH_VERSION', '4.21.4' );
