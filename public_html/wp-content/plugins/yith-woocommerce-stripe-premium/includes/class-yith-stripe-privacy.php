...
if ( ! class_exists( 'YITH_WCStripe_Privacy' ) ) {
	...
	class YITH_WCStripe_Privacy extends YITH_Privacy_Plugin_Abstract {
		...
		public function get_privacy_message( $section ) {
			...
			switch ( $section ) {
				case 'collect_and_store':
					$content = '<p>' . __( 'While you visit our site, we’ll track:', 'yith-woocommerce-stripe' ) . '</p>' .
							'<ul>' .
							'<li>' . __( 'Card unique identifiers: these IDs, returned by Stripe when submitting card details, will be stored to help you check out faster the next time you make a purchase on our store.', 'yith-woocommerce-stripe' ) . '</li>' .
			...
			/**
			 * APPLY_FILTERS: yith_wcstripe_privacy_policy_content
			 *
			 * Filters privacy policy HTML content.
			 *
			 * @param string $content Default value depending in the section.
