...
if ( ! class_exists( 'YWSBS_Subscription' ) ) {
	...
	class YWSBS_Subscription extends YWSBS_Subscription_Legacy {
		 ...
		 *
		 * @since 1.0.0
		 * @param int   $subscription_id (Optional) Subscription ID. Default is 0.
		 * @param array $args            (Optional) Arguments. Default is empty array.
		 */
		...
		*/
		...
		/**
		 * Populate array prop without save data.
		 *
		 * @param string|array $prop  Property name or an array of prop => value.
		 * @param mixed        $value Value of property.
		 */
		public function populate_prop( $prop, $value = null ) {
			if ( is_array( $prop ) ) {
				foreach ( $prop as $prop_name => $prop_value ) {
					$this->populate_prop( $prop_name, $prop_value );
				}
			} else {
				$this->array_prop[ $prop ] = $value;
			}
		...
		*/
		...
		public function get_id( $context = '' ) {
			if ( ! empty( $context ) ) {
				_deprecated_argument( __METHOD__, '3.0.0' );
			}
		 ...
		 * @return WP_Post
		 */
		public function get_post( $context = '' ) {
			if ( ! empty( $context ) ) {
				_deprecated_argument( __METHOD__, '3.0.0' );
			}
		...
		/**
		 * Returns the trial_per for this object.
		 *
		 * @since  2.0.0
		 * @param string $context What the value is for. Valid values are view and edit.
		 * @return string
		 */
		public function get_trial_per( $context = 'view' ) {
			return $this->get( 'trial_per', $context );
		}

		/**
		 * Returns the trial_time_option for this object.
		 *
		 * @since  2.0.0
		 * @param string $context What the value is for. Valid values are view and edit.
		 * @return string
		 */
		public function get_trial_time_option( $context = 'view' ) {
			return $this->get( 'trial_time_option', $context );
		}
			...
			if ( $new_status !== $old_status || ! in_array( $new_status, array_keys( ywsbs_get_status() ), true ) ) {
				...
				switch ( $new_status ) {
					...
					case 'resume':
						...
						$payment_due_date = apply_filters( 'ywsbs_subscription_resume_payment_due_date', $this->get_payment_due_date() + $offset, $product, $this );
					...
					case 'cancel-now':
						...
						do_action( 'ywsbs_customer_subscription_cancelled_mail', $this );
						// translators: %s: Who set the new status request.
						YITH_WC_Activity()->add_activity( $this->id, 'cancelled', 'success', $this->get( 'order_id' ), sprintf( esc_html_x( 'The subscription has now been cancelled. %s', '%s: Who set the new status request', 'yith-woocommerce-subscription' ), $from_text ) );
						break;
					case 'expired':
						$this->set( 'status', $new_status );
						$this->cancel_renew_order();
		...
		public function update_prices( $posted ) {
			...
			if ( $changes ) {
				...
				foreach ( $changes as $key => $change ) {
					if ( 'quantity' !== $key ) {
						$currency     = get_woocommerce_currency_symbol( $this->get( 'order_currency' ) );
						$price_format = get_woocommerce_price_format();

						$new_value = sprintf( $price_format, $currency, wc_format_decimal( $new_values[ $key ], '' ) );
						$old_value = sprintf( $price_format, $currency, wc_format_decimal( $old_values[ $key ], '' ) );
					} else {
					...
					$messages[] = sprintf( _x( '%1$s from %2$s to %3$s', '1: Field changed, 2: Old value, 3: New value', 'yith-woocommerce-subscription' ), ucfirst( str_replace( '_', ' ', $key ) ), $old_value, $new_value );
				}
				// translators: placeholder: dynamic list of changes.
				YITH_WC_Activity()->add_activity( $this->id, 'changed', 'success', 0, sprintf( esc_html_x( 'Changed subscription data: %s ', 'placeholder: list of changes', 'yith-woocommerce-subscription' ), '<br>' . implode( '<br>', $messages ) ) );
			}
		...
		*/
		...
		public function start( $order_id ) {
			...
			$order = wc_get_order( $order_id );
			// do nothing if this subscription has payed with this order.
			if ( empty( $order ) || ( ! empty( $payed ) && in_array( $order->get_id(), $payed, true ) ) ) {
				return;
			}
			...
			$this->set( 'payed_order_list', array( $order->get_id() ) );
			// if there's an upgrade/downgrade.
			$subscription_to_cancel_info = $order->get_meta( '_ywsbs_subscritpion_to_cancel' );
		...
		public function cancel( $now = true ) {
			...
			$note = sprintf( __( 'This order has been cancelled because the subscription %s has been cancelled', 'yith-woocommerce-subscription' ), $this->get_number() );
		...
		*/
		...
		public function can_be_paused() {
			...
			if ( is_array( $pause_info ) && 'yes' === $pause_info['allow_pause'] && $this->has_status( 'active' ) && ( 0 === $pause_info['max_pause'] || ( $pause_info['max_pause'] > 0 && $this->get_num_of_pauses() < $pause_info['max_pause'] ) ) ) {
				$can_be_paused = true;
			}
		...
		public function can_be_cancelled() {
			...
			} elseif ( ! $this->has_status( array( 'pending', 'overdue', 'suspended', 'cancelled' ) ) ) {

				$product              = $this->get_product();
				$product_override     = $product instanceof WC_Product && 'yes' === $product->get_meta( '_ywsbs_override_cancellation_settings' );
				$can_be_cancelled_opt = $product_override ? $product->get_meta( '_ywsbs_can_be_cancelled' ) : get_option( 'ywsbs_allow_customer_cancel_subscription', 'yes' );

				if ( 'limited' === $can_be_cancelled_opt ) {
					// Get limit.
					if ( $product_override ) {
						$can_be_cancelled_limit = $product->get_meta( '_ywsbs_cancellation_limit' );
					} else {
						$days_after_creation    = get_option( 'ywsbs_cancel_subscription_limit_after_creation', array( 'days' => 0 ) );
						$days_before_renew      = get_option( 'ywsbs_cancel_subscription_limit_before_renew', array( 'days' => 0 ) );
						$can_be_cancelled_limit = array(
							'days_after_creation' => absint( $days_after_creation['days'] ),
							'days_before_renew'   => absint( $days_before_renew['days'] ),
						);
					}

					$can_be_cancelled =
						( empty( $can_be_cancelled_limit['days_after_creation'] ) || ( $this->get_start_date() + ( DAY_IN_SECONDS * $can_be_cancelled_limit['days_after_creation'] ) ) < current_time( 'timestamp' ) ) &&
						( empty( $can_be_cancelled_limit['days_before_renew'] ) || ( $this->get_next_payment_due_date() - ( DAY_IN_SECONDS * $can_be_cancelled_limit['days_before_renew'] ) ) > current_time( 'timestamp' ) );

				} else {
					$can_be_cancelled = 'yes' === $can_be_cancelled_opt;
				}
			}
		...
		public function can_be_create_a_renew_order() {
			...
			$status                = array( 'pending', 'expired', 'paused' );

			// exit if no valid subscription status.
			if ( $this->has_status( $status ) || $this->get_payment_due_date() === $this->get_expired_date() ) {
				yith_subscription_log( 'A renew order cannot be created because the subscription is  ' . $this->get( 'status' ), 'subscription_payment' );
