...
if ( ! class_exists( 'YWSBS_Subscription_Cron' ) ) {
	...
	class YWSBS_Subscription_Cron {
			...
			if ( ! $lock ) {
				...
				if ( ! empty( $subscriptions ) ) {
					foreach ( $subscriptions as $subscription_post ) {
						...
						$time = apply_filters( 'ywsbs_current_time', time() );
						...
						if ( ywsbs_scheduled_actions_enabled() && $subscription->get_payment_due_date() > $time ) {
							...
							yith_subscription_log( 'Payment due date for subscription ' . $subscription_post->ID . ' set to ' . $subscription->get_payment_due_date(), 'subscription_payment' );
							yith_subscription_log( 'Time: ' . $time, 'subscription_payment' );
		...
		public function ywsbs_check_subscription_payment() {
			...
			$new_status                 = $after_renew_order_creation['status'] ?? 'suspended';
		...
		public function ywsbs_cancel_subscription_expired() {
			...
			if ( ! empty( $subscriptions ) ) {
				foreach ( $subscriptions as $subscription_post ) {
					...
					YITH_WC_Activity()->add_activity( $subscription->get_id(), 'expired', 'success', $subscription->get_order_id(), __( 'The subscription has been cancelled because it expired.', 'yith-woocommerce-subscription' ) );
				}
			}
			...
			if ( ! $lock ) {
				...
				$renew_orders_for_first_time = wc_get_orders(
					array(
						...
						'status'           => $status,
						'date_created'     => '<' . $from,
						'ywsbs_meta_query'   => array( // phpcs:ignore
							array(
								'key'   => 'failed_attemps',
								'value' => '0',
							),
				...
				if ( $renew_orders ) {
					foreach ( $renew_orders as $renew_order ) {
						if ( ywsbs_scheduled_actions_enabled() ) {
							$renew_order = wc_get_order( $renew_order );
							if ( $renew_order ) {
							...
							YWSBS_Subscription_Order()->pay_renew_order( $renew_order, $is_manual_renew );
						}
					}
				}
		...
		public function ywsbs_trigger_email_before_subscription_expired() {
			...
			if ( ! empty( $subscriptions ) ) {
				foreach ( $subscriptions as $subscription_post ) {
                    if ( 'yes' !== get_post_meta( $subscription_post->ID,  '_ywsbs_before_expired_email_sent', true ) ) {
                        $subscription = ywsbs_get_subscription( $subscription_post->ID );
		...
		public function ywsbs_trash_pending_subscriptions() {
			...
			if ( ! empty( $subscriptions ) ) {
				foreach ( $subscriptions as $subscription ) {
					...
					$subscription->trash();
					YITH_WC_Activity()->add_activity( $subscription_id, 'trashed', 'success', $order, __( 'The subscription has been deleted after the set duration for being in "Pending" status.', 'yith-woocommerce-subscription' ) );
				}
			}
		}
		...
		public function ywsbs_trash_cancelled_subscriptions() {
			...
			if ( ! empty( $subscriptions ) ) {
				foreach ( $subscriptions as $subscription ) {
					...
					$order           = $subscription->get( 'order_id' );
					$subscription->trash();
					YITH_WC_Activity()->add_activity( $subscription_id, 'trashed', 'success', $order, __( 'The subscription has been deleted after the set duration for being in "Cancelled" status.', 'yith-woocommerce-subscription' ) );
				}
			}
		}
	}
