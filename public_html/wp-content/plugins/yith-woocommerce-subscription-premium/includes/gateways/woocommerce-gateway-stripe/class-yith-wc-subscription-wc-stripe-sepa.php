...
/**
 * YITH_WC_Subscription_WC_Stripe_Sepa integration with WooCommerce Stripe Plugin
 *
 * @class   YITH_WC_Subscription_WC_Stripe_Sepa
 ...
 */
class YITH_WC_Subscription_WC_Stripe_Sepa extends WC_Gateway_Stripe_Sepa {
	...
	public function __construct() {
		...
		// Disable check renew order for SEPA payments.
		add_filter( 'ywsbs_check_the_renew_order', array( $this, 'disable_check_renew_order' ), 10, 2 );
	}
		...
		foreach ( $subscriptions as $subscription_id ) {
			...
			try {
				...
				if ( ! empty( $response->error ) && ! $is_authentication_required ) {
					...
					ywsbs_register_failed_payment( $renewal_order, $localized_message );
	...
	/**
	 * Disable check renew order for SEPA payments.
	 * This to prevent sent multiple intent for the same order.
	 *
	 * @param bool               $disabled True if disabled, false otherwise.
	 * @param YWSBS_Subscription $subscription The subscription object.
	 * @return bool
	 */
	public function disable_check_renew_order( $disabled, $subscription ) {
		return $disabled && $this->id !== $subscription->get_payment_method();
	}
