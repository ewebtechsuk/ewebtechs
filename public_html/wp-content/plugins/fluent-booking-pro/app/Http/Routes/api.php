...
use FluentBooking\App\Http\Policies\MeetingPolicy;
...
use FluentBookingPro\App\Http\Controllers\TransactionController;
...
$router->prefix('schedules')->withPolicy(MeetingPolicy::class)->group(function ($router) {
    $router->put('{id}/transactions/{transaction_id}', [TransactionController::class, 'updateTransaction'])->int('id')->int('transaction_id');
});
...
$router->prefix('calendars')->withPolicy(CalendarPolicy::class)->group(function ($router) {
    ...
    $router->get('/{id}/events/{event_id}/payment-settings', [PaymentMethodController::class, 'getPaymentSettings'])->int('id')->int('event_id');
    $router->post('/{id}/events/{event_id}/payment-settings', [PaymentMethodController::class, 'updatePaymentSettings'])->int('id')->int('event_id');
