...
class HMWPP_Models_Menu {
	...
	public function getMenu( $menu ) {
		...
		if ( ! isset( $menu['hmwp_twofactor'] ) ) {
			$splitIndex = 7;
			$menu       = array_merge( array_slice( $menu, 0, $splitIndex ), $hmwpp_menu, array_slice( $menu, $splitIndex ) );
		} else {
	...
	public function getFeatures( $features ) {
		...
		$feature = array(
			'title'       => esc_html__( "Magic Link Login", 'hide-my-wp-pack' ) ,
		...
		$feature = array(
			'title'       => esc_html__( "Woocommerce Magic Link", 'hide-my-wp-pack' ),
			'description' => esc_html__( "Allow users to log in to the website using their email address and a unique login URL delivered via email.", 'hide-my-wp-pack' ),
			'free'        => true,
			...
			'logo'        => 'fa fa-clock-o',
			'link'        => false,
			'details'     => HMWPP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/magic-link-login/#ghost-woocommerce-login',
		...
		if ( ! array_search( 'hmwp_2falogin', array_column( $features, 'option' ) ) ) {
			$features[] = array(
				...
				'details'     => HMWPP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/two-factor-authentication/',
				'show'        => true,
			);
		}
