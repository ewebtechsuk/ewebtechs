...
class HMWPP_Models_Twofactor {
	...
	public function validateTwoFactor() {
		...
		if ( ( $backupService->isServiceActive( $user ) && $backupService->validateAuthentication( $user ) ) || ( $twoFactorService->isServiceActive( $user ) && $twoFactorService->validateAuthentication( $user ) ) || ( $emailService->isServiceActive( $user ) && $emailService->validateAuthentication( $user ) ) ) {
		...
		$this->loginHtml( $user, $login_nonce['key'], $redirect_to, esc_html__( 'ERROR: Invalid verification code.', 'hide-my-wp-pack' ) );
		exit;
	...
	public function showNotices( $user ) {
		...
		if ( $user_failures ) {
			...
			), HMWPP_Classes_Tools::getOption( 'hmwp_2falogin_fail_message' ) );
			echo '</strong></div>';
		}
	}
	...
	public function adminNotices() {
		...
		if ( ! $backupService->isServiceActive( $user ) ) {
            ...
            <div class="error">
				...
				<span>
					<?php
					echo wp_kses( sprintf( __( '2FA Codes: You are out of backup codes and need <a href="%s">new codes!</a>', 'hide-my-wp-pack' ), esc_url( get_edit_user_link( $user->ID ) . '#hmwp_two_factor_options' ) ), array( 'a' => array( 'href' => true ) ) );
					?>
				<span>
                </p>
            </div>
	...
	/**
	 * Sort the users table by LastLogin
	 *
