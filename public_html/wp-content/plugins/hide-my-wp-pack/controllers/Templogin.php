...
class HMWPP_Controllers_Templogin extends HMWPP_Classes_FrontController {
	...
	public function hookInit() {
		...
		if ( class_exists( 'HMWP_Classes_ObjController' ) && HMWP_Classes_ObjController::getClassByPath( 'HMWP_Controllers_Templogin' ) ) {
			return;
		}
			...
			foreach ( $users as $user ) {
				...
				if ( $user->details->is_active ) {
					$form .= '<form method="POST" class="col-3 m-0 p-1">
                                ...
                                <button type="submit" class="btn btn-link btn-sm m-0 p-0" /><i class="fa fa-unlock" title="' . esc_attr__( 'Lock user', 'hide-my-wp-pack' ) . '"></i></button>
                            </form>';
				} else {
					$form .= '<form method="POST" class="col-3 m-0 p-1">
                                ...
                                <input type="hidden" name="user_id" value="' . $user->ID . '" />
                                <button type="submit" class="btn btn-link btn-sm m-0 p-0" /><i class="fa fa-lock" title="' . esc_attr__( 'Reactivate user for 1 day', 'hide-my-wp-pack' ) . '"></i></button>
                            </form>';
				}

				$form .= '<div class="col-3 m-0 p-1"><a href="?page=hmwp_templogin&action=hmwp_update&user_id=' . esc_attr( $user->ID ) . '" type="button" class="btn btn-link btn-sm m-0 p-0" /><i class="fa fa-edit" title="' . esc_attr__( 'Edit user', 'hide-my-wp-pack' ) . '"></i></a></div>';
				$form .= '<form method="POST" class="col-3 m-0 p-1">
                                ' . wp_nonce_field( 'hmwpp_templogin_delete', 'hmwp_nonce', true, false ) . '
                                <input type="hidden" name="action" value="hmwpp_templogin_delete" />
                                <input type="hidden" name="user_id" value="' . $user->ID . '" />
                                <button type="submit" class="btn btn-link btn-sm m-0 p-0" onclick="return confirm(\'' . esc_attr__( 'Do you want to delete temporary user?', 'hide-my-wp-pack' ) . '\')" /><i class="fa fa-close text-danger" title="' . esc_attr__( 'Delete user', 'hide-my-wp-pack' ) . '"></i></button>
                            </form>';
				if ( $user->details->is_active ) {
					$form .= '<div class="col-3 m-0 p-1"><button type="button" id="text-' . $user->ID . '" class="btn btn-link btn-sm m-0 p-0 hmwp_clipboard_copy" data-clipboard-text="' . $user->details->templogin_url . '" /><i class="fa fa-link" title="' . esc_attr__( 'Copy Link', 'hide-my-wp-pack' ) . '"></i></button></div>';
				}
