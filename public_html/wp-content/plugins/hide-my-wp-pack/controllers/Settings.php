...
class HMWPP_Controllers_Settings extends HMWPP_Classes_FrontController {
	...
	public function init() {
		...
		echo '<noscript><div class="alert-danger text-center py-3">' . sprintf( esc_html__( "Javascript is disabled on your browser! You need to activate the javascript in order to use %s plugin.", 'hide-my-wp-pack' ), esc_attr( HMWPP_Classes_Tools::getOption( 'hmwp_plugin_name' ) ) ) . '</div></noscript>';
		...
		if ( ! empty( $logs ) ) {
			foreach ( $logs as $log ) {
				...
				if ( ( esc_attr( $user->user_email ) ) ) {
					$user_details .= '<p class="inline-block pt-1 font-medium text-black-50">' . esc_html( $user->user_email ) . '</p>';
				}
