...
class HMWPP_Classes_Tools {
	...
	public static function getOptions() {
		...
		if ( ! $options['hmwp_2falogin_message'] ) {
			$options['hmwp_2falogin_message'] = "ERROR: Too many invalid verification codes, you can try again in {time}.";
		}
		if ( ! $options['hmwp_2falogin_fail_message'] ) {
			$options['hmwp_2falogin_fail_message'] = "WARNING: Your account has attempted to login {count} times without providing a valid code. The last failed login occurred {time} ago. If this wasn't you, please reset your password.";
		}
	...
	public static function getValue( $key = null, $defaultValue = false, $keep_newlines = false ) {
		...
		$ret = ( isset( $_POST[ $key ] ) ? $_POST[ $key ] : ( isset( $_GET[ $key ] ) ? $_GET[ $key ] : $defaultValue ) ); // phpcs:ignore WordPress.Security, handled in the next lines

		if ( is_string( $ret ) === true ) {
			if ( $keep_newlines === false ) {

				if ( $key == 'hmwp_2falogin_message' ) { //validate message
					$ret = preg_replace( '/[^A-Za-z0-9-_!.\s]/', '', $ret );
				} elseif ( $key == 'redirect_to' ) { //validate url parameter
	...
	public static function getIsset( $key = null ) {
		...
		return isset( $_POST[ $key ] ) || isset( $_GET[ $key ] ); // phpcs:ignore WordPress.Security, check isset only
	}

	/**
	 * Check if WP multisite
	 *
	 * @return bool
	 */
	...
	/**
	 * Check if WP multisite with path
	 *
	 * @return bool
	 */
