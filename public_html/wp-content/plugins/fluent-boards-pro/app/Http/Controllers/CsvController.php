...
use FluentBoards\Framework\Request\Request;
...
class CsvController extends Controller
{
    ...
    public function importBoard()
    {
        ...
        //creating stages
        $stageIds = [];
        if (!array_key_exists('stage', $allTasksToImport[0])) {
        ...
        $message = __('Board has been imported successfully', 'fluent-boards');
    ...
    private function createBoard($task)
    {
        if (array_key_exists('board_title', $task) && $task['board_title']) {
            ...
            $boardName = 'Imported - Board name ' . date("Y-m-d");
        }
        $boardData = [
            'title' => $boardName,
    ...
    {
        $currentStages = Stage::where('board_id', $board->id)->get();
        ...
        foreach ($tasks as $index => $task) {
            ...
            if (array_key_exists('subtasks', $task) && $task['subtasks']) {
                ...
                foreach ($subtasks as $subtask) {
                    ...
                    $board->tasks()->create($subtaskData);
                }
            }
        }
    }
