...
### Example: filling in secrets from the Hostinger FTP panel

From the Hostinger control panel screenshot you provided:

1. Go to **GitHub → Settings → Secrets and variables → Actions → New repository secret** for each secret below.
2. Enter the exact values from Hostinger, **without** adding the `ftp://` prefix to the hostname:
   - `HOSTINGER_FTP_HOST` → `82.208.208.79`
   - `HOSTINGER_FTP_USERNAME` → `u758780470@ewebtechs.com`
   - `HOSTINGER_FTP_PASSWORD` → (click **Change FTP password** in Hostinger, copy the generated password, and paste it here)
   - `HOSTINGER_FTP_PORT` → `21`
   - `HOSTINGER_FTP_SERVER_DIR` → `public_html/`
3. Because the Hostinger panel labels the connection as `ftp://`, also add `HOSTINGER_FTP_PROTOCOL` with the value `ftp` so the workflow uses plain FTP instead of FTPS.
4. Save each secret. The next push to `main` that includes files under `public_html/` will deploy automatically.
